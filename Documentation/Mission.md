...
Pipeline CI/CD au format YAML (ou équivalent)

Analyse du Data Drift  au format notebook
...
Durant la présentation orale, l’évaluateur interprétera le rôle de Chloé votre Lead Data Scientist chez "Prêt à Dépenser".

La soutenance durera 30 minutes et sera structurée de la manière suivante :
...
Le contexte de votre mission.

Les résultats du monitoring : Présentez votre analyse sur la dérive des données (graphiques, métriques) et comment vous collectez/analysez les logs ou autres métriques pertinentes (performance, erreurs).

Les optimisations de performance : Résumez les tests effectués, les goulots d'étranglement identifiés et l'amélioration concrète obtenue sur le temps de réponse/inférence.

La structure de votre dépôt GitHub : Naviguez rapidement dans votre dépôt pour montrer l'organisation du code, le Dockerfile, les fichiers de configuration CI/CD et la documentation (README).

Vous réaliserez les démonstrations suivantes :

Le fonctionnement de l'API déployée : Envoyez une requête type à votre API et montrez la réponse (le score prédit).

Le fonctionnement du pipeline CI/CD : Montrez comment un changement dans le code (ex: commit Git) déclenche automatiquement les tests, la construction de l'image Docker et le déploiement de l'API.

2. Discussion (10 minutes)

L’évaluateur jouera le rôle de Chloé. Il vous challengera sur les points suivants :

Robustesse et fiabilité : Gestion des erreurs à tous les niveaux.

Monitoring et maintenance : Gestion de la dérive des données, maintenance à long terme.

Optimisation et scalabilité : Bonne compréhension des choix software / hardware. 

3. Débrief (5 minutes)

À la fin de la soutenance, l'évaluateur arrêtera de jouer le rôle de Chloé pour vous permettre de débriefer ensemble sur votre travail, les points forts, les axes d'amélioration et la validation des compétences.
